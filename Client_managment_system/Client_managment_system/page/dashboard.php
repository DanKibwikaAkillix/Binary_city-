...
<body>
  <div class="container">
    ...
    <table class="table table-bordered table-striped">
      ...
      <tbody>
        <?php
        // Fetch clients ordered by name and their respective contacts count
        $stmt = $conn->prepare("SELECT c.name, c.code, COUNT(co.client_code) AS contact_count
                                  ...
                                  GROUP BY c.code
                                  ORDER BY c.name ASC"); // Order by name ASC
        $stmt->execute();
        ...
        if ($result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) {
            echo "<tr>
                    ...
                    <td><button class='btn btn-info' onclick=\"openModal('{$row['code']}')\">View Contacts</button></td>
                  </tr>";
          }
        } else {
          ...
          <div id="contactsBody">
            ...
            if (isset($_GET['client_code'])) {
              ...
              if ($result->num_rows > 0) {
                ...
                echo "<thead><tr><th>Name</th><th>Surname</th><th>Email</th></tr></thead><tbody>";
                while ($row = $result->fetch_assoc()) {
